...
export class StrategyEngine {
  ...
  private simulateTrades(candles: Candlestick[], config: StrategyConfig, strategyId: string): Trade[] {
    ...
    for (let index = startIndex; index < candles.length; index++) {
      ...
      if (!position && short > long) {
        position = { entryPrice: candle.close, entryTime: candle.openTime };
        continue;
      }
  ...
  private calculateMetrics(trades: Trade[], candles: Candlestick[]) {
    if (trades.length === 0) {
      return [
