...
import type { MarketPerpetualSymbol, MarketServerMessage, MarketSnapshotPayload, MarketUpdatePayload } from '@/types/market';
import { fetchPerpetualSymbols, fetchSupportedIntervals } from '@/services/market';
import { useMarketSocket } from '@/services/marketSocket';
...
export const useMarketStore = defineStore('market', () => {
  ...
  const perpetualSymbols = ref<MarketPerpetualSymbol[]>([]);
  ...
  void (async () => {
    try {
      const [intervalList, symbolsList] = await Promise.all([
        fetchSupportedIntervals(),
        fetchPerpetualSymbols(),
      ]);
      if (intervalList.length > 0) {
        supportedIntervals.value = intervalList;
      }
      if (symbolsList.length > 0) {
        perpetualSymbols.value = symbolsList;
      }
    } catch (error) {
      console.error('Failed to bootstrap market metadata', error);
    }
  })();
  ...
  return {
    ...
    colors,
    interval,
    intervalDurations: supportedIntervals,
    candles,
    isLoaded,
    activeMarkers,
